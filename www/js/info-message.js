AFRAME.registerComponent('info-message', {
  ...
  addStyles: function () {
    var css =
      '.a-info-message{border-radius: 10px; position: fixed; width: ' + this.data.width + 'px;' +
      'height: ' + this.data.height + 'px; background-color: white; border: 3px solid rgba(0,0,0,0.65);' +
      'top: 22px; left: 22px; color: rgb(51, 51, 51); padding: 20px 15px 0 15px;' +
      'font-size: 11pt; line-height: 20pt; z-index: 400; overflow-y: scroll !important;}' +

      '.a-info-message a{border-bottom: 1px solid rgba(53,196,232,.15); color: #1497b8;' +
      'position: relative; text-decoration: none; transition: .1s ease;}' +

      '@media only screen and (min-width: 1200px) {' +
      '.a-info-message {font-size: 12pt}}' +

      '@media only screen and (max-width: 600px) {' +
      '.a-info-message {left: 20px; right: 20px; top: 60px; width: auto}}' +

      '@media only screen and (max-height: 600px) {' +
      '.a-info-message {left: 20px; top: 20px; height: 250px}}' +
      ...
      '.a-info-message-container {position: absolute; left: 100px; top: 20px;}' +
      '.a-info-message-button {background: rgba(0, 0, 0, 0.35) ' + this.infoMessageButtonDataURI + ' 50% 50% no-repeat;}' +
      '.a-info-message-button {background-size: 92% 90%; border: 0; top: 10px; cursor: pointer; min-width: 78px; min-height: 34px; padding-right: 0; padding-top: 0; position: fixed; left: 10px; transition: background-color .1s ease; -webkit-transition: background-color .05s ease; z-index: 400; border-radius: 8px; touch-action: manipulation;}' +
